...
Architecture: Phase 10.3 rebuild — copied from agents_old/tools/ticket_lookup_tool.py
...
# SEC-05: GBV/sensitive ticket details never exposed via this tool.
"""
import logging
...
def _lookup_ticket_impl(user_id: str, tracking_number: str = "") -> dict[str, Any]:
    ...
    assert user_id, (
        "user_id is required for ticket lookup — "
        "cannot look up tickets without authenticated user"
    )
...
class LookupTicketInput(BaseModel):
    """Input schema for lookup_ticket_tool."""
    user_id: str = Field(
        ...,
        description="The authenticated user's UUID. MANDATORY. Never empty.",
    )
    tracking_number: str = Field(
        "",
        description=(
            "Optional tracking number (e.g. TKT-20260218-A1B2C3). "
            "If empty, returns up to 10 most recent tickets."
        ),
    )
...
class LookupTicketTool(BaseTool):
    ...
    description: str = (
        "Look up ticket status for a citizen, scoped strictly to their own tickets. "
        "Requires user_id (the authenticated citizen's UUID). "
        "Optionally filter by tracking_number for a specific ticket."
    )
...
lookup_ticket_tool = LookupTicketTool()
