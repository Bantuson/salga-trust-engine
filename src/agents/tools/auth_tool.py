...
Pattern: implementation function (_impl) + BaseTool wrapper with Pydantic input schema.

# POPIA: _log_tool_failure() truncates user ID to 8 chars — no PII in logs.
# SEC-01: Auth tools stateless — no class/instance state preserving PII between calls.
"""
import logging
import random
import string
...
# 5-minute sliding window, CRITICAL log at 3+ failures
# POPIA: never log full user identifiers — truncated to 8 chars
...
def _log_tool_failure(tool_name: str, error: str, user_identifier: str) -> None:
    ...
    # POPIA: user_identifier truncated to 8 chars — no PII in logs.
...
def _get_twilio_client():
    ...
    if _twilio_client is None:
        if settings.TWILIO_ACCOUNT_SID and settings.TWILIO_AUTH_TOKEN:
            ...
            logger.warning(
                "Twilio client not configured (missing TWILIO_ACCOUNT_SID or TWILIO_AUTH_TOKEN)"
            )
...
def _lookup_user_impl(phone_or_email: str) -> str:
    ...
    # SEC-01: Admin client bypass of RLS is intentional here — auth tools
    # need cross-tenant visibility to find citizens by phone/email.
...
class LookupUserInput(BaseModel):
    ...
    phone_or_email: str = Field(
        ...,
        description="E.164 phone number (e.g. +27831234567) or email address",
    )
...
def _send_otp_impl(
    ...
    Args:
        ...
        is_returning_user: If True, sets should_create_user=False (prevents duplicate
            account creation for returning citizens)
    ...
    if is_phone:
        ...
        if twilio is None:
            return (
                "Error: Twilio SMS client not configured. "
        ...
        try:
            ...
            twilio.messages.create(
                body=(
                    f"Your SALGA Trust Engine verification code is: {code}. "
...
class SendOtpInput(BaseModel):
    ...
    channel: str = Field(
        "sms",
        description="Delivery channel — 'sms' for phone or 'email' for email OTP",
    )
    is_returning_user: bool = Field(
        False,
        description=(
            "If True, sets should_create_user=False "
            "(prevents duplicate account creation for returning citizens)"
        ),
    )
...
class VerifyOtpInput(BaseModel):
    ...
    phone_or_email: str = Field(
        ...,
        description="E.164 phone or email address used in send_otp_tool",
    )
    otp_code: str = Field(..., description="6-digit code the user provided")
    otp_type: str = Field(
        "sms",
        description="OTP type — 'sms' for phone, 'email' for email OTP",
    )
...
def _create_supabase_user_impl(
    ...
    #         without auth agent verification flow completing first.
...
class CreateUserInput(BaseModel):
    ...
    phone_or_email: str = Field(
        ...,
        description="E.164 phone number or email address (used as identifier)",
    )
    full_name: str = Field(..., description="Citizen's full name for their profile")
    tenant_id: str = Field(
        ...,
        description="UUID of the municipality the citizen belongs to",
    )
    preferred_language: str = Field(
        "en",
        description="Language preference — 'en', 'zu', or 'af'",
    )
    residence_verified: bool = Field(
        False,
        description="Whether proof of residence has been verified",
    )
    secondary_contact: str = Field(
        "",
        description="Secondary email or phone number",
    )
    address: str = Field(
        "",
        description="Residential address from proof of residence",
    )
