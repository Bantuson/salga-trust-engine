...
import { getHeatmapData } from '../../services/publicApi';
...
export function HeatmapViewer({ municipalityId }: HeatmapViewerProps) {
  ...
  useEffect(() => {
    async function loadHeatmapData() {
      setIsLoading(true);
      const data = await getHeatmapData(municipalityId);
      setPoints(data);
      setIsLoading(false);
    }
    loadHeatmapData();
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        Heatmap shows aggregated report density. Individual addresses are not displayed to protect privacy.
      </div>
    </div>
  );
}
