 ...
 *
 * Uses react-dropzone for file selection and Supabase Storage for uploads.
...
import { confirmUpload } from '../services/api';
...
export function FileUpload({
  ...
  const onDrop = async (acceptedFiles: File[], rejectedFiles: any[]) => {
      ...
      setUploadStatuses((prev) =>
        prev.map((status) =>
          status.status === 'uploading'
            ? { ...status, status: 'error', error: 'Authentication error' }
            : status
        )
      );
      return;
    ...
    for (let i = 0; i < acceptedFiles.length; i++) {
      ...
      try {
        // Determine bucket based on GBV flag
        ...
        await confirmUpload(fileId, 'evidence');
