...
must_haves:
  truths:
    ...
    - "Mobile hamburger menu shows user avatar, Profile link, and Sign Out when authenticated"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    **Changes:**
    ...
    4. **Mobile menu (authenticated state):**
       When user exists, the mobile hamburger menu overlay should show the user's identity and auth-specific links. This was identified as needing explicit detail during the Playwright UI audit — the mobile menu is the primary navigation on phones where the desktop dropdown is hidden.

       Add a user identity block at the TOP of the mobile nav (above existing nav links), plus Profile and Sign Out at the bottom:
       ```tsx
       {/* Inside mobile-nav-overlay, before the NavLink list */}
       {user && (
       ...
       {/* Existing NavLinks: Home, Dashboard, About, My Reports */}

       {/* After existing nav links, add auth-only links */}
       {user && (
         <>
           <NavLink to="/profile" className={({ isActive }) => `mobile-nav-link ${isActive ? 'active' : ''}`} onClick={() => setIsMobileMenuOpen(false)}>
             Profile
           </NavLink>
       ...
       **Also replace the mobile CTA when authenticated:**
       The existing mobile "Report Issue" button at the bottom of mobile nav should conditionally show:
       - Unauthenticated: existing Report Issue CTA button (links to /report which triggers auth gate)
       - Authenticated: no separate CTA needed (Report Issue is in the nav links above via Profile/My Reports flow)

       **Add CSS to App.css for mobile auth styles:**
       ```css
  ...
  <verify>
    ...
    Verify App.css has .header-user-dropdown and .mobile-user-identity styles.
    Verify mobile menu shows user avatar and name when authenticated.
  </verify>
  <done>
    PublicHeader shows user menu dropdown when authenticated (avatar, name, dropdown with My Reports, Profile, Report Issue, Sign Out). Unauthenticated state unchanged. Click-outside closes dropdown. Mobile hamburger menu shows user avatar + name at top, Profile link, and Sign Out button when authenticated.
  </done>
</task>
...
7. Clicking "My Reports" when not logged in redirects to /login
</verification>
