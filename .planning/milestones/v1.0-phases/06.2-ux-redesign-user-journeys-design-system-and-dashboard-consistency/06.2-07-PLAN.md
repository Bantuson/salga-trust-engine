...
type: execute
wave: 4
depends_on: ["06.2-02", "06.2-03", "06.2-04", "06.2-05", "06.2-06", "06.2-08"]
files_modified:
...
must_haves:
  truths:
    ...
    - "Backend integration verified: access request, onboarding, invitations, and citizen portal all communicate with API endpoints"
    - "All new backend endpoints are reachable and return expected responses"
  artifacts:
  ...
  <how-to-verify>
    **Start the public dashboard:**
    ...
    4. **My Reports** (/my-reports): Verify:
       ...
       - Personal stats cards fetched from backend (total, resolved, avg time)
       - Regular ticket cards with full details from backend
       - GBV case card: lock icon, limited info, emergency numbers (10111, 0800 150 150)
       - Skeleton loaders while data loads
       - Network tab shows GET /api/v1/citizen/my-reports and GET /api/v1/citizen/stats calls
    ...
    **Start the municipal dashboard:**
    ...
    7. **Request Access page** (/request-access): Verify:
       ...
       - Submission calls POST /api/v1/access-requests (check Network tab)
       - Success confirmation shown after backend response
    ...
    9. **Onboarding** (/onboarding): Verify:
       ...
       - Each step saves to backend (check Network tab for PUT /api/v1/onboarding/state/{step})
       - Team invitations submit to backend (check Network tab for POST /api/v1/invitations)
       - Skip works for optional steps
       - Wizard can be resumed after page refresh (data loaded from backend)
       - Completion celebration and POST /api/v1/onboarding/complete call
