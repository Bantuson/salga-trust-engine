...
must_haves:
  truths:
    ...
    - "Upload confirm endpoint is called after each file upload to create MediaAttachment DB records (verified: src/api/v1/uploads.py confirm_upload creates MediaAttachment with file_id query param)"
  artifacts:
...
**Important: Do NOT change any UI/JSX.** Only modify the TypeScript logic (interfaces, handlers, constants).
  ...
  <verify>
Verify upload confirm endpoint contract matches: `grep -n "def confirm_upload\|file_id.*str\|purpose.*str\|MediaAttachment" src/api/v1/uploads.py` — must show file_id: str and purpose: str as query params and MediaAttachment creation.
