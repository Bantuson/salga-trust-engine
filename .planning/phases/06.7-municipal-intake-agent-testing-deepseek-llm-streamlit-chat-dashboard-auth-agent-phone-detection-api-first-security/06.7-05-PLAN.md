...
must_haves:
  truths:
    ...
    - "Streamlit is a TESTING tool — developer-focused, not citizen-facing"
  artifacts:
...
Use synchronous httpx (not async) — Streamlit's execution model is synchronous. Set a generous timeout (120s) because LLM calls via CrewAI can take 30-60 seconds.
...
- `httpx.TimeoutException` -> return `{"reply": "Request timed out. The LLM may be slow — try again.", ...}`
...
st.caption("Developer testing tool — connects to live crew server")
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Municipality picker (free text for now — could be dropdown from DB)
...
def render_debug_panel():
    ...
    status_color = {"active": "🟢", "expired": "🟡", "none": "🔴", "created": "🟢"}.get(session_status, "⚪")
    ...
    with st.expander("Raw Debug Data", expanded=False):
        # Check if this is a GBV scenario — redact content
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <action>
Human verification of the complete end-to-end flow. No code changes — verify that all components work together.
  </action>
  <verify>
...
3. In the browser:
   ...
   e. Try the "Municipal" scenario preset — verify it auto-populates a water pipe message
