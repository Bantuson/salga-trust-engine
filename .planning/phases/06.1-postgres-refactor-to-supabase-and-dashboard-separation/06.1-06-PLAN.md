...
must_haves:
  truths:
    ...
    - "Public dashboard is fully serverless (zero backend dependency)"
  artifacts:
...
Purpose: Per locked decision, the public dashboard is fully serverless: React on Vercel + Supabase JS client + PostgreSQL via RLS. No FastAPI intermediary. This enables independent deployment and scaling. The anon key + RLS views ensure only aggregated, non-sensitive data is accessible.

Output: Complete frontend-public/ app with municipality selector, response time charts, resolution rate charts, geographic heatmap, all powered by Supabase direct queries.
</objective>
