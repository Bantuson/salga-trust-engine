...
must_haves:
  truths:
    ...
    - "OTP template is deployed to both 'Magic Link' and 'Confirm signup' Supabase Dashboard slots"
  artifacts:
...
<task type="auto">
  ...
  <action>
**SQL Remediation (run first via Supabase MCP):**

Execute this SQL via the `mcp__supabase__execute_sql` tool:
```sql
UPDATE auth.users
...
- OTP input should have centered text with letter spacing for code-entry feel
  ...
  <done>Both login pages support email OTP sign-in mode alongside existing email+password and phone OTP. Stuck accounts confirmed via SQL. Both frontends compile without TypeScript errors.</done>
</task>
