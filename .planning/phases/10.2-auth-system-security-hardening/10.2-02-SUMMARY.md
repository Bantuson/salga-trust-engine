...
plan: 02
subsystem: auth
tags: [password-validation, security, react, typescript, client-side, frontend]
...
provides:
  ...
  - "Supabase Dashboard security settings checklist for human verification"
...
**Real-time password requirements UI on both registration pages with inline hint indicators matching the 12-char backend policy**

## Performance

- **Duration:** ~12 min
- **Started:** 2026-02-25T08:10:23Z
- **Completed:** 2026-02-25T08:22:00Z
- **Tasks:** 1 of 2 (paused at Task 2 checkpoint — human verification required)
- **Files modified:** 2
...
2. **Task 2: Verify Supabase Dashboard security settings** - PENDING (checkpoint:human-verify)
...
**Task 2 (Checkpoint) — Supabase Dashboard verification required:**

The user must manually verify and configure the following Supabase Dashboard settings:

1. **Authentication > Providers > Email:**
   - Confirm "Enable email confirmations" is ON
   - Confirm OTP Length is 8 digits
   - Set OTP Expiry to 600 seconds (10 minutes) if currently higher

2. **Authentication > Advanced:**
   - JWT expiry should be 3600 seconds (1 hour)
   - Refresh token reuse interval should be default (10 seconds)

3. **Authentication > Passwords** (if available on plan):
   - Set minimum password length to 12
   - Enable require lowercase, uppercase, digits
   - Leave symbols OFF
   - If Pro Plan: enable HaveIBeenPwned check, set reuse prevention to 5

4. **Authentication > Sessions** (if Pro Plan):
   - Set inactivity timeout to 24 hours

5. **Assess `email_confirm: True` workaround** in `src/api/v1/auth.py` line 109:
   - If SMTP is configured: change to `email_confirm: False` for proper email verification
   - If SMTP not configured: leave as-is and note as remaining hardening item

**Resume signal:** Type "dashboard verified" followed by findings (plan tier, SMTP status, settings applied, email_confirm decision)

## Next Phase Readiness
- Client-side password validation complete and committed
- Awaiting Supabase Dashboard verification checkpoint (Task 2) before Plan 02 can be marked complete
- After checkpoint: SEC-01/SEC-02/SEC-04 requirements will be fully verified
...
- VERIFIED: `validatePassword` present 4 times in each file
- VERIFIED: 12-char requirement enforced in both files

---
*Phase: 10.2-auth-system-security-hardening*
*Completed: 2026-02-25 (Task 1 only — Task 2 awaiting human checkpoint)*
